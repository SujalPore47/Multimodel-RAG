...
###################################################################################################################################
    ...
    if 'img_file' in st.session_state and st.sidebar.button("Stop chatting with Image file"):
        del st.session_state['img_file']  # Remove from session state
...
###################################################################################################################################
    ...
    # Initialize voice_recording to avoid UnboundLocalError
    voice_recording = None

    # Creating the form for input
    st.write("## Send a Message")

    # Creating the form for input
    with st.form(key="input_form", clear_on_submit=False):
        # Text area for entering or displaying messages, initialized with session state
        input_text = st.text_area("Enter your message:", value=st.session_state['input_text'], key="input_text")

        # Creating columns for buttons
        input_columns = st.columns([0.5, 0.5, 0.5, 0.5])
        ...
        with input_columns[1]:
            # Button for web search
            search_button = st.form_submit_button("Search The Web", on_click=handle_search)

        with input_columns[2]:
            # Button for summarizing video content
            summarise_button = st.form_submit_button("Summarise the video", on_click=handle_summarise)
